...
func TestCost_TradingCosts(t *testing.T) {
	...
	// for Antoine
	costOneStone := NewTradingPrice(bruno, nil)[Stone]
	assert.Equal(t, Coins(4), costOneStone)

	// for Bruno
	costOneStone = NewTradingPrice(antoine, nil)[Stone]
